...
"""Generate configuration for tests"""
import os
import configparser
...
def _generate_base_config():
    """Generate base config for other configs"""
    # Account key
...
def generate_acme_dns_tiny_config():
    """Generate config for acme_dns_tiny with domain and keys"""
    # Simple configuration with good options
    account_key, domain_key, _, config = _generate_base_config()
    ...
    account_key, domain_key, domain_csr, config = _generate_base_config()
    os.remove(domain_key)

    cname_csr = domain_csr
...
def generate_acme_account_rollover_config():
    """Generate a new config with two account keys to roll over them"""
    # Old account is directly created by the config generator
    old_account_key, domain_key, _, config = _generate_base_config()
    os.remove(domain_key)
    ...
    rollover_account_config = NamedTemporaryFile(delete=False)
    with open(rollover_account_config.name, 'w') as configfile:
        config.write(configfile)

    return {
        # config and keys (returned to keep files on system)
        "config": rollover_account_config.name,
        "old_account_key": old_account_key,
        "new_account_key": new_account_key.name
    }

def generate_acme_account_deactivate_config():
    """Generate a new config with a new account key to delete it"""
    # Account key is created by the by the config generator
    account_key, domain_key, _, config = _generate_base_config()
    os.remove(domain_key)

    deactivate_account_config = NamedTemporaryFile(delete=False)
    with open(deactivate_account_config.name, 'w') as configfile:
        config.write(configfile)

    return {
        "config": deactivate_account_config.name,
        "key": account_key
    }
