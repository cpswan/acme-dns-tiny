#!/usr/bin/env python3
"""Test the acme_dns_tiny script with real ACME server"""
import unittest
import sys
import os
import subprocess
...
class TestACMEDNSTiny(unittest.TestCase):
    ...
    def _assert_cert_chain(self, certificate_chain):
        # Output have to contains two certiicates
        certlist = certificate_chain.split("-----BEGIN CERTIFICATE-----")
    ...
    def test_success_cn(self):
        ...
        self._assert_cert_chain(certchain)
    ...
    def test_success_cn_with_csr_option(self):
        ...
        acme_dns_tiny.main(["--csr", self.configs['cnameCSR'],
    ...
    def test_success_dnshost_ip(self):
        ...
        self.assertIn("INFO:acme_dns_tiny:A and/or AAAA DNS resources not found \
for configured dns host: we will use either resource found if one exists \
or directly the DNS Host configuration.", adnslog.output)
        certchain = sys.stdout.getvalue()
    ...
    def test_success_cli(self):
        ...
        certout, _ = subprocess.Popen([
            ...
            "--verbose"], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def test_success_cli_with_csr_option(self):
        ...
        certout, _ = subprocess.Popen([
            "python3", "acme_dns_tiny.py", "--csr", self.configs['cnameCSR'],
            self.configs['goodCNameWithoutCSR'], "--verbose"
    ...
    def test_weak_key(self):
        ...
        self.assertRaisesRegex(ValueError, "key too small", acme_dns_tiny.main,
                               [self.configs['weakKey'], "--verbose"])
    ...
    def test_failure_dns_update_tsigkeyname(self):
        ...
        self.assertRaisesRegex(ValueError, "Error updating DNS",
