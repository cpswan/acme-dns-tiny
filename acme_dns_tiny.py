...
def get_crt(config, log=LOGGER):
    ...
    def _send_signed_request(url, payload):
        ...
        jose = {"protected": protected64, "payload": payload64, "signature": _b64(signature)}
        try:
...
either resource found if one exists or directly the DNS Host configuration.")
    ...
    jws_header = _jws_from_key(config["acmednstiny"]["AccountKeyFile"])
    ...
    if http_response.status_code == 201:
        ...
        log.info("  - Registered a new account: '%s'", jws_header["kid"])
    elif http_response.status_code == 200:
        jws_header["kid"] = http_response.headers['Location']
        log.debug("  - Account is already registered: '%s'", jws_header["kid"])
    ...
    for authz in order["authorizations"]:
        ...
        try:# A CNAME resource can be used for advanced TSIG configuration
            ...
            dnsrr_domain = [response.to_text()
                            for response in resolver.query(dnsrr_domain, rdtype="CNAME")][0]
...
will install TXT directly on %s", dnsrr_domain, type(dnsexception).__name__)
        ...
        log.info("Waiting for 1 TTL (%s seconds) before starting self challenge check.",
...
%s : %s", type(dnsexception).__name__, dnsexception)
    ...
    joseheaders['Accept'] = config["acmednstiny"].get("CertificateFormat",
